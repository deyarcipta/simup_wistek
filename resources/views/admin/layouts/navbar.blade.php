  ...
  <div class="navbar-nav align-items-center d-none d-md-flex">
    <div class="nav-item d-flex align-items-center gap-2">
      <span class="badge bg-label-primary px-3 py-2 rounded-pill fw-semibold">
        <i class="bx bx-check-circle me-1"></i> SIMUP Wistek Active
      </span>
    ...
    <li class="nav-item dropdown-user dropdown">
      <a class="nav-link dropdown-toggle hide-arrow d-flex align-items-center gap-2" href="#" data-bs-toggle="dropdown">
        <div class="d-none d-md-flex flex-column text-end me-1">
          <span class="fw-bold text-dark" style="font-size: 0.9rem; line-height: 1.2;">{{ Auth::user()->name ?? session('name', 'Admin') }}</span>
          <small class="text-muted" style="font-size: 0.75rem; text-transform: capitalize;">{{ Auth::user()->role ?? session('role', 'Administrator') }}</small>
        </div>
        ...
        <li class="p-2 border-bottom mb-1">
          <div class="d-flex align-items-center gap-2">
            <div class="flex-grow-1">
              <span class="fw-bold d-block text-dark">{{ Auth::user()->name ?? session('name', 'Admin') }}</span>
              <small class="text-muted">{{ Auth::user()->email ?? 'admin@wistek.sch.id' }}</small>
            </div>
          </div>
        </li>
        <li>
          <a class="dropdown-item rounded-3 py-2" href="{{ route('admin.profile.edit') }}">
            <i class="bx bx-user me-2 text-primary"></i> Edit Profile
          </a>
        </li>
        <li>
          <a class="dropdown-item rounded-3 py-2" href="{{ route('pengaturan.index') }}">
            <i class="bx bx-cog me-2 text-info"></i> Pengaturan
          </a>
        </li>
